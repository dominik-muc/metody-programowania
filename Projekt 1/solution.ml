...
let rec choose m n = if m > n then [] else m :: choose (m + 1) n
  ...
  | p :: ps ->
      ...
      if ps = [] then
        ...
        @ builder
            (available - a - p)
            (sum - p) ps
            (insert_n_variables p true (insert_n_variables a false acc))
      else
        [ acc ]
        @ builder
            (available - a - p - 1)
            (sum - p) ps
            (false :: insert_n_variables p true (insert_n_variables a false acc))
...
let parse_row (xs : bool list) : int list =
  let rec helper xs curr acc : int list =
    ...
    | x :: xs ->
        if x then helper xs (curr + 1) acc
        else helper xs 0 (if curr = 0 then acc else curr :: acc)
  in
  helper xs 0 []
...
let rec verify_rows (pss : int list list) (xss : bool list list) =
  match (pss, xss) with
  | ps :: pss, xs :: xss ->
      if verify_row ps xs then verify_rows pss xss else false
  | _ -> true
...
let rec transpose xss =
  match xss with [] -> [] | xs :: xss -> append_vertical xs (transpose xss)
...
let example_1 = { rows = [ [ 2 ]; [ 1 ]; [ 1 ] ]; cols = [ [ 1; 1 ]; [ 2 ] ] }
